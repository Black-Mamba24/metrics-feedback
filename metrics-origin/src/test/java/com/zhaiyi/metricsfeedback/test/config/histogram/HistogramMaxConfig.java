 ...
 */
public class HistogramMaxConfig implements FeedbackConfig {
    @Override
    public FeedbackConfiguration config(FeedbackConfiguration.Builder builder) {
        return builder.type(MetricType.HISTOGRAM).metricName(NameUtil.getMetricName(MetricTest.class, "histogramMax"))
                .setThreshold(HistogramConstants.max, 10, 100).initialDelay(0).period(1).timeUnit(TimeUnit.SECONDS)
                .action(new Action() {
                    @Override
