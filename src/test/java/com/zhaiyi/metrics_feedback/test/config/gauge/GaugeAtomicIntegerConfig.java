 ...
 */
public class GaugeAtomicIntegerConfig implements FeedbackConfig {

    @Override
    public FeedbackConfiguration config(FeedbackConfiguration.Builder builder) {
        return builder.type(MetricType.GAUGE).metricName(NameUtil.getMetricName(MetricTest.class, "gaugeAtomicInteger"))
                .setThreshold(MetircConstants.value, -10, 25).initialDelay(0).period(1).timeUnit(TimeUnit.SECONDS)
                .action(new Action() {
                    @Override
